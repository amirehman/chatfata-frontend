    ...
    <div class="inner">
      ...
      >
        ...
        <input
          ...
          :class="[
            searchMode ? 'focusinput block shadow-xl' : 'block',
            mobile === 'true' ? 'p-2' : 'p-3'
          ]"
        />
        ...
        >
          <template v-if="term.length < 1">
            <p
              class="py-6 text-gray-600 tracking-widest text-left pl-16 text-sm xxl:text-base"
            >
          ...
          <template v-else>
            ...
            <ul v-else>
              <li
                class="search-recipe-li"
                v-for="(recipe, u) in SearchRecipes"
                :key="u"
              >
                <SearchRecipe :recipe="recipe" :mobile="mobile" />
              </li>
              <li class="search-recipe-li" v-if="SearchRecipes.length > 8">
                <nuxt-link
                class="tracking-widest p-3 hover:bg-gray-100 block focus:outline-none focus:bg-gray-100 dark:focus:dark-mode-light" to="/search"
                  >View All </nuxt-link
                >
              </li>
            </ul>

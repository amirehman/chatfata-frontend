<template>
  <div>
      <client-only>
        <Keypress key-event="keydown" :key-code="27" @success="closeSideNav" />
      </client-only>
    <div
      class="full-screen-menu-wrapper fixed w-full h-screen transition duration-500 ease-out"
      :class="isMenuOpen ? 'dark:bg-dark-mode-light bg-gray-100 z-50 block' : 'z--50'"
    >
      <div
        class="absolute top-0 -left-32 opacity-0 w-5/6 sm:w-1/2 lg:w-1/3 xl:w-1/4 dark:bg-dark-mode bg-white h-screen shadow-xl dark:border-gray-700 border-l transition duration-300 ease-out transform "
        :class="isMenuOpen ? 'z-50 translate-x-32rem opacity-100' : 'translate-x-0'"
      >
        <div class="z-50 absolute top-0 right-0 pr-5 pt-5 smw-close-button">
          <div class="inner">
            <ul class>
              <li>
                <span class="cursor-pointer text-gray-700 dark:text-gray-400" @click="closeSideNav">
                    <svg class="fill-current w-6 xxl:w-8 h-6 xxl:h-8 hover:w-12 hover:h-12" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M10 8.586L2.929 1.515 1.515 2.929 8.586 10l-7.071 7.071 1.414 1.414L10 11.414l7.071 7.071 1.414-1.414L11.414 10l7.071-7.071-1.414-1.414L10 8.586z"/></svg>
                </span>
              </li>
            </ul>
          </div>
        </div>
        <!-- smw close button end -->

          <div class="here p-5 absolute w-full  mt-12">
            <SearchRecipes mobile="true" />
          </div>

        <div class="navbar-wrapper h-screen flex items-center justify-center">
          <ul class="text-lg xxl:text-xl capitalize text-gray-800 dark:text-gray-400">
            <li class="mb-2">
              <nuxt-link
                to="/recipes"
                class="hover:text-theme-orange transition duration-300"
              >All Recipes.</nuxt-link>
            </li>
            <li class="mb-2">
              <nuxt-link to="/ingredients" class="hover:text-theme-orange transition duration-300">
                Browse By
                ingredients.
              </nuxt-link>
            </li>
            <li class="mb-2">
              <a href="/about" class="hover:text-theme-orange transition duration-300">About.</a>
            </li>
            <li class="mb-2">
              <a
                href="contact.html"
                class="hover:text-theme-orange transition duration-300"
              >Contact</a>
            </li>

            <li class="mb-2">
              <nuxt-link
                to="/login"
                class="hover:text-theme-orange transition duration-300"
              >login</nuxt-link>
            </li>

            <li class="divider w-full bg-gray-300 dark:bg-gray-700 h-1 mt-12 mb-5"></li>

            <li class="socia-links">
              <ul class="flex items-center justify-start">
                <li class="mx-2">
                  <a href>
                    <svg
                      class="w-5 h-5 fill-current text-gray-700 dark:text-gray-400 hover:text-theme-orange transtion duration-300"
                      viewBox="0 0 24 24"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M23.998 12c0-6.628-5.372-12-11.999-12C5.372 0 0 5.372 0 12c0 5.988 4.388 10.952 10.124 11.852v-8.384H7.078v-3.469h3.046V9.356c0-3.008 1.792-4.669 4.532-4.669 1.313 0 2.686.234 2.686.234v2.953H15.83c-1.49 0-1.955.925-1.955 1.874V12h3.328l-.532 3.469h-2.796v8.384c5.736-.9 10.124-5.864 10.124-11.853z"
                      />
                    </svg>
                  </a>
                </li>
                <li class="mx-2">
                  <a href>
                    <svg
                      class="w-5 h-5 fill-current text-gray-700 dark:text-gray-400 hover:text-theme-orange transtion duration-300"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                    >
                      <path
                        d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913a5.885 5.885 0 001.384 2.126A5.868 5.868 0 004.14 23.37c.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558a5.898 5.898 0 002.126-1.384 5.86 5.86 0 001.384-2.126c.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913a5.89 5.89 0 00-1.384-2.126A5.847 5.847 0 0019.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227a3.81 3.81 0 01-.899 1.382 3.744 3.744 0 01-1.38.896c-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421a3.716 3.716 0 01-1.379-.899 3.644 3.644 0 01-.9-1.38c-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678a6.162 6.162 0 100 12.324 6.162 6.162 0 100-12.324zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405a1.441 1.441 0 01-2.88 0 1.44 1.44 0 012.88 0z"
                      />
                    </svg>
                  </a>
                </li>
                <li class="mx-2 mr-0">
                  <a href>
                    <svg
                      class="w-6 h-6 fill-current text-gray-700 dark:text-gray-400 hover:text-theme-orange transtion duration-300"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                    >
                      <path
                        d="M23.495 6.205a3.007 3.007 0 00-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 00.527 6.205a31.247 31.247 0 00-.522 5.805 31.247 31.247 0 00.522 5.783 3.007 3.007 0 002.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 002.088-2.088 31.247 31.247 0 00.5-5.783 31.247 31.247 0 00-.5-5.805zM9.609 15.601V8.408l6.264 3.602z"
                      />
                    </svg>
                  </a>
                </li>
              </ul>
            </li>
            <!-- social link end -->

            <li class="lowercase absolute w-full bottom-0 left-0 text-center mb-24 text-base">
              <a
                class="transition duration-300 inline-block pb-1 dark:hover:border-gray-700 border-b border-transparent hover:border-gray-500"
                href="mailto:hi@amirr.net"
              >info@shallos-fry.com</a>
            </li>
          </ul>
        </div>
        <!-- navbar-wrapper end -->
      </div>
    </div>
    <!-- full screen menu wrapper end -->
  </div>
</template>

<script>

import SearchRecipes from "../search/SearchRecipes.vue"

export default {
  components :{
      Keypress: () => import('vue-keypress'),
      SearchRecipes
  },
  data() {
    return {
    }
  },
  computed: {
    storageUrl() {
      return this.$store.state.storageUrl;
    },
    isMenuOpen () {
      return this.$store.state.sideNav
    }
  },
  methods: {
    closeSideNav() {
      this.$store.commit("mutateSideNavStatus", false);
    }
  }
};
</script>
